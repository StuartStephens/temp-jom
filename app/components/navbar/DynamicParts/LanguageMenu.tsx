...
function getLocaleHomepage(locale: string, siteInfo?: WebsiteInfo) {
  if (!siteInfo?.locales?.includes(locale)) return "/";

  const domains: { domain: string; defaultLocale: string }[] =
    siteInfo?.localeDomains || [];
  const domain = domains.filter((x) => x.defaultLocale === locale);
  if (domain && domain.length > 0) return domain[0].domain;

  if (!siteInfo?.primaryDomain) return `/${locale}/`;
...
export const LanguageMenu: FunctionComponent<LanguageMenuProps> = (props) => {
  ...
  function handleClick(event: MouseEvent<HTMLElement>, locale: string) {
    var languageUrl = new URL(getLocaleHomepage(locale, website?.data));
    if (contentData?.data) {
      const contentLinks =
        contentData?.data?.existingLanguages?.filter((x) => x.name == locale) ??
        [];
  ...
  return (
    <Fragment>
      ...
      >
        <MenuItem selected={false} divider={true} disabled={true}>
          Language:
        </MenuItem>
        {website?.data?.labels.map((label) => (
          <MenuItem
            key={label.code}
            onClick={(e) => handleClick(e, label.code)}
            selected={locale == label.code}
          >
            <ListItemIcon>
              <BookmarkBorderIcon />
            </ListItemIcon>
...
const LanguageMenuFallback: FunctionComponent<{}> = () => (
  <IconButton size="small" color="inherit" aria-haspopup="false">
    <LanguageIcon sx={{ fontSize: "40px" }} />
  </IconButton>
);

export const LanguageMenuWithErrorBoundary = withErrorBoundary(
  LanguageMenu,
